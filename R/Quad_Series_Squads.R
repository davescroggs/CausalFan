...
game_register <- list(roundNumber = 1:3, matchNumber = 1:2) %>%
cross_df() %$%
map2(roundNumber,matchNumber,~downloadMatch("11695",.x,.y))

player_info <- game_register %>%
  map_dfr( ~ .x[["playerInfo"]][["player"]]) %>% 
  distinct(playerId,displayName,surname)

team_info <- game_register %>%
  map_dfr( ~ .x[["teamInfo"]][["team"]]) %>% 
  distinct(squadId,squadNickname)

subs <- tibble(subs = game_register %>%
                 map(~ .x[["playerSubs"]][["player"]]),
               round = game_register %>% map_int(~.x[["matchInfo"]][["roundNumber"]]),
               match = game_register %>% map_int(~.x[["matchInfo"]][["matchNumber"]])) %>%
  unnest_longer("subs") %>% unnest_wider("subs") %>%
  left_join(game_register %>%
              map_dfr( ~ .x[["teamInfo"]][["team"]]) %>% distinct(squadNickname, squadId),
            by = "squadId")

starting_pos <- tibble(subs = game_register %>%
...
schedule <- tibble(squadId = map_int(game_register,~ .x[["matchInfo"]][["homeSquadId"]]),
                       opposition = map_int(game_register,~ .x[["matchInfo"]][["awaySquadId"]]),
                       round = map_int(game_register,~.x[["matchInfo"]][["roundNumber"]]),
                       match = map_int(game_register,~.x[["matchInfo"]][["matchNumber"]])) %>% 
  {bind_rows(.,rename(.,opposition = squadId,squadId = opposition))}

goals <- tibble(subs = game_register %>%
...
positions_per_time <- bind_rows(
  ...
  subs %>%
    ...
    mutate(type = "sub") %>% 
    select(playerId,round,match,squadId,position = toPos,period,periodSeconds,type),
  # starting positions
  starting_pos %>% 
    mutate(periodSeconds = 0,type = "pos") %>% 
    rename(position = startingPositionCode)) %>% 
  arrange(round,match,squadId,playerId,period,periodSeconds) %>%
  group_by(squadId,playerId,match,round,period) %>% 
...
positions_per_time %>% 
  ...
  filter(round == 1,squadNickname == "Vitality Roses") %>% View()
...
# Player combinations -----------------------------------------------------
  ...
  combos <- starting_pos %>% 
    #filter(round == 1,match == 1,squadId == 833) %>% 
    mutate(time = 0L) %>% 
